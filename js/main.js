const apiKey = 'Pxq66GvfEzXiZmQAioHPa30rPleV43Md'
...
const searchImage = document.getElementById('searchImageLens')
...
async function getSuggestions(event) {
    ...
    try{
        const response = await fetch(`https://api.giphy.com/v1/gifs/search/tags?api_key=${apiKey}&q=${input.value.toLowerCase()}`)
...
async function showMore(word) {
    try {
        const response = await fetch(`https://api.giphy.com/v1/gifs/search?api_key=${apiKey}&q=${word.toLowerCase()}&offset=${number + 12}`)
